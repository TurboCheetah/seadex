...
const Boolean = ({ value, title }: { value: boolean; title: string }) => {
    return (
        <ListItem>
            <ListItemIcon>{value ? <DoneIcon /> : <CloseIcon />}</ListItemIcon>
            <ListItemText primary={title} />
        </ListItem>
    )
}
const ReleaseLinkListItem = ({
    link,
    site,
}: {
    link?: string
...
export default function ReleaseDetailsList({ release }: { release: Release }) {
    return (
        <List>
            ...
            <Boolean
                value={release.isExclusiveRelease}
                title="Exclusive Release"
            />
            <Boolean value={release.isBroken} title="Broken" />
            {release.notes && (
                <NestedList icon={NotesIcon} text="Notes">
                    <Typography sx={{ pl: 4, pr: 2, py: 1 }}>
                        {release.notes}
                    </Typography>
                </NestedList>
            )}
            {release.comparisons && (
                <NestedList icon={CompareIcon} text="Comparisons">
                    <Typography sx={{ pl: 4, pr: 2, py: 1 }}>
                        {release.comparisons}
                    </Typography>
                </NestedList>
